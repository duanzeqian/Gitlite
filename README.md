...
`content`：文件的二进制内容（vector<unsigned char>）。
...
感觉这个`README`写到这里就差不多了，这个项目的核心难点就在于设计和封装~~（start from Scratch）~~。援引伟大的gwb学长原话来说就是~~（龚✌✌的恩情还不完）~~，“gitlite是教你设计和封装的”“我说白了，这个大作业是教你OOP的，类不是用来简化代码的，是有核心思想在的，为什么一定要这个抽象层”。
